...
namespace UI
{
    public partial class frmClientePesquisar : Form
    {
        ...
        //Esta propriedade devolve o ID Cliente para o formulario anterior
        public int IdCliente { get; set; }

        private void frmClientesPesquisar_Load(object sender, EventArgs e)
        {
            //TESTAR SE EXISTE REGISTROS NA TABELA          
            var lista = MetodosCliente.LeituraTodosClientes();

            if (lista == null)
            {
                MessageBox.Show("Nenhum Cliente Cadastrado", "Aviso");
        ...
        public void CarregarGrid()   //http://www.macoratti.net/10/11/c_efcrud.htm
        {
            try
            {
                ...
                IEnumerable<Cliente> lista =
                    from c in contexto.ClienteCtx orderby c.Nome select c;
        ...
        {
            frmClientesPesquisar_Load(sender, e);
        }
